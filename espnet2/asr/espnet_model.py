...
class ESPnetASRModel(AbsESPnetModel):
        ...
        if self.use_transducer_decoder:
            ...
            if self.ctc_weight != 1.0:
                loss_att, acc_att, cer_att, wer_att = self._calc_att_loss(
                    encoder_out, encoder_out_lens, text, text_lengths, kwargs.get('prompt', None)
                )
    ...
    def _calc_att_loss(
        ...
        if prompt is not None:
            prompt[prompt==self.ignore_id] = self.eos
            # Can replace with  torch.cat([prompt[:, 1:], [self.sos], ys_out_pad],dim=-1)
            ys_out_pad = torch.cat([prompt[:, 1:], torch.full((prompt.shape[0], 1), self.sos, device=ys_out_pad.device), ys_out_pad],dim=-1)
            ys_in_pad = torch.cat([prompt, ys_in_pad],dim=-1)
            ys_in_lens += prompt.shape[-1]
