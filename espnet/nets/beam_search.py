...
class BeamSearch(torch.nn.Module):
    ...
    def forward(
        ...
        """
        def delete_element(arr, idxs):
            for idx in idxs:
                arr = torch.cat([arr[:idx], arr[idx+1:]], dim=0)
            return arr
        # set length bounds
        ...
        use_label = label is not None
        if prompt is not None:
            prompt = prompt.tolist()
        if prefix is not None:
            prefix = prefix.tolist()
            
        running_hyps = self.init_hyp(x= x if pre_x is None else pre_x, prompt=prompt, prefix=prefix)
        ended_hyps = []
        if use_label:
            maxlen = min(maxlen, label.shape[1])
        allow_tokens = None
        all_tokens = np.arange(self.n_vocab)
        for i in range(maxlen):
            
            logging.debug("position " + str(i))
            if use_label:
                allow_tokens = torch.from_numpy(np.delete(all_tokens.copy(), label[:,i])).to(x.device)
                
            best = self.search(running_hyps, x, pre_x=pre_x, allow_tokens=allow_tokens)
            # post process of one iteration
            running_hyps = self.post_process(i, maxlen, maxlenratio, best, ended_hyps)
            # Force the allow tokens to be consistent with previous tokens
            if i == 0 and use_label and len(running_hyps) != 0:
                label = label[label[:,i]==running_hyps.yseq[0,-1].item()]
            # end detection
