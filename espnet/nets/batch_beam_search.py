...
class BatchBeamSearch(BeamSearch):
    ...
    def init_hyp(self, x: torch.Tensor, prompt: List[int] = None, prefix: List[int] = None) -> BatchHypothesis:
        ...
        if prompt is not None:
            primer = prompt +  primer
            
        if prefix is not None:
            primer = primer + prefix
        return self.batchfy(
            [
                Hypothesis(
                    score=0.0,
    ...
    def search(
        ...
        x: torch.Tensor,
        pre_x: torch.Tensor = None,
        allow_tokens: torch.Tensor = None,
    ) -> BatchHypothesis:
        ...
        # Force the decoder to pick given answer
        # Only for bigsuperb decoding
        if allow_tokens is not None:
            weighted_scores = weighted_scores.index_fill_(
                1, 
                allow_tokens, 
                float('-inf')
            )
            # for i in range(len(weighted_scores[0])):
            #     if i not in allow_tokens:
            #         weighted_scores[0, i] = torch.tensor(float('-inf'), device=weighted_scores.device)
