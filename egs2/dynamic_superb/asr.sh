...
use_label=false # Use text prompt ids for multi lingual multi tasking
...
Options:
    ...
    --use_label     # Whether to use label to help decoding (default="${use_nlp_prompt}").
...
if [ ${stage} -le 12 ] && [ ${stop_stage} -ge 12 ] && ! [[ " ${skip_stages} " =~ [[:space:]]12[[:space:]] ]]; then
    ...
    for dset in ${_dsets}; do
        ...
        if ${use_prompt} && ${use_nlp_prompt};
        then
            log "Use nlp prompt"
            _opts+="--data_path_and_name_and_type '${_data}/emptytext,text,text' "
            _opts+="--data_path_and_name_and_type '${_data}/prompt,prompt,text' "
        fi
        if ${use_label};
        then
            log "Use label to help decoding"
            _opts+="--data_path_and_name_and_type '${_data}/label,label,text' "
        fi
        # shellcheck disable=SC2086
        utils/split_scp.pl "${key_file}" ${split_scps}
